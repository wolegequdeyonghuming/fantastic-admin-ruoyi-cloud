...
import type { MenuForm, MenuQuery, MenuVO } from '@/api/modules/system/menu/types'
import {
  addMenu,
  cascadeDelMenu,
  delMenu,
...
const { success: toastSuccess, warning: toastWarning } = useFaToast()
...
async function getList() {
  queryParams.value.systemCode = route.query.systemCode
  loading.value = true
...
async function getTreeselect() {
  menuOptions.value = []
  const response = await listMenu({ systemCode: route.query.systemCode })
  const menu: MenuOptionsType = { menuId: 0, menuName: '主类目', children: [] }
  // 使用 handleTree 工具函数处理树形数据
  const { useSettingsStore } = await import('@/store/modules/settings')
  const settingsStore = useSettingsStore()
  const handleTree = settingsStore.handleTree
...
function submitForm() {
  menuFormRef.value?.validate(async (valid: boolean) => {
    if (valid) {
      form.value.systemCode = route.query.systemCode
      form.value.menuId
...
onMounted(() => {
  loadDicts('sys_show_hide', 'sys_normal_disable')
  sys_show_hide.value = getDictOptions('sys_show_hide')
  sys_normal_disable.value = getDictOptions('sys_normal_disable')
  getList()
})
</script>
    ...
    <el-card shadow="hover">
      <template #header>
        <el-row :gutter="10">
          ...
          <el-col :span="1.5">
            <el-button type="warning" plain icon="Close" @click="resetBack">
              关闭
            </el-button>
          </el-col>
          <right-toolbar
